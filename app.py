...
STATIC_DIR = 'static'
INVOICE_DIR = '/tmp/invoices'  # Vercel writable path
...
app.secret_key = os.getenv('SECRET_KEY', 'hakvenlong-secret-2025')
...
def notify_paid(t):
    if t.get('paid_notified') or not TELEGRAM_API: return
    try:
        lines = ["PAID ORDER", t['name'], f"{t['phone']} | {t['address']}", f"{t['total_khr']:,.0f} KHR", f"Bill: `{t['bill_number']}`"]
...
@app.route('/shop') 
def shop(): return render_template('shop.html', products=products)
...
def checkout():
    ...
    if request.method == 'POST':
        sel = [int(x.strip()) for x in request.form.get('selected_items', '').split(',') if x.strip().isdigit()] or [i['id'] for i in cart]
        selected_cart = [i for i in cart if i['id'] in sel]
        total_price = sum(i['price'] * i['quantity'] for i in selected_cart)
        name, email, phone, address = request.form.get('name','').strip(), request.form.get('email','').strip(), request.form.get('phone','').strip(), request.form.get('address','').strip()
        ...
        if payment_method == 'qr':
            ...
            khqr = KHQR(token)
            qr_data = khqr.create_qr(bank_account=merchant_bank_account, merchant_name=merchant_name, merchant_city=merchant_city,
                                     amount=total_khr, currency='KHR', bill_number=bill_number, phone_number=merchant_phone,
                                     store_label="HAK VENLONG SHOP", terminal_label="ONLINE STORE")
            md5 = khqr.generate_md5(qr_data)
            qr_path = os.path.join(STATIC_DIR, 'qr_codes', f"{md5}.png")
            qrcode.make(qr_data).save(qr_path)

            session['transaction'] = {'md5': md5, 'total_khr': total_khr, 'bill_number': bill_number, 'name': name, 'email': email,
                              'phone': phone, 'address': address, 'cart': selected_cart, 'paid_notified': False, 'invoice_generated': False}
            session.modified = True
...
def check_payment_status():
    ...
    status = KHQR(token).check_payment(md5)
    if status == "PAID":
        notify_paid(t)
        if not t.get('invoice_generated'):
            pdf = generate_pdf_invoice(t, t['cart'])
            t['invoice_path'], t['invoice_generated'] = pdf, True
            session['transaction'] = t; session.modified = True
        session['cart'] = []
...
def download_invoice():
    path = session.get('transaction', {}).get('invoice_path')
    if not path or not os.path.exists(path): return "Not ready", 404
    return send_file(path, as_attachment=True, download_name=f"invoice_{session['transaction']['bill_number']}.pdf")

@app.route('/customer_thanks')
def customer_thanks():
    t = session.get('transaction', {})
    return render_template('customer_thanks.html', name=t.get('name','Customer'), address=t.get('address',''),
                           bill_number=t.get('bill_number','N/A'), amount_khr=t.get('total_khr',0),
                           paid=bool(t.get('paid_notified')), has_invoice=bool(t.get('invoice_generated')))
...
def add_to_cart():
    data = request.get_json(); pid, qty = int(data['id']), int(data['quantity'])
...
def update_cart_quantity():
    data = request.get_json(); cart = session.get('cart', [])
    for item in cart:
...
def remove_from_cart():
    cart = session.get('cart', [])
    session['cart'] = [i for i in cart if i['id'] != int(request.get_json()['id'])]
    session.modified = True
    return jsonify({'status': 'success'})
